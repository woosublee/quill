...
struct SystemDefaultAndSystemAudioRecorderSourceTests {
    static func main() throws {
        ...
        precondition(source.contains("group.notify(queue: .global(qos: .userInitiated))"), "mixdown should not run on the main queue")
        precondition(source.contains("let finalURL = self.finalRecordingURL(microphoneURL: urls.microphoneURL, systemAudioURL: urls.systemAudioURL)"))
