...
`LocalAIModelStore` starts from the `NativeWhisperModelStore` lifecycle pattern—`.download`-suffixed partial files, a 95% size floor, SHA-256 validation, model-scoped status, and install/delete operations—but extends it to packages with multiple artifacts. A model is ready only when every artifact validates; interrupted multi-file replacements use scoped backup files that are recovered or cleaned before the next install/startup, and deleting a model removes its finals, partials, and recognized transaction backups without touching another model. `LocalAIInstaller` likewise retains Whisper's cancellation token, streaming progress, and in-flight de-duplication shape while downloading a model's artifacts sequentially, aggregating package progress, and committing or rolling back the package as one logical installation. The types remain parallel and independently testable rather than forcing a premature generic abstraction over two lifecycles that now differ materially.
